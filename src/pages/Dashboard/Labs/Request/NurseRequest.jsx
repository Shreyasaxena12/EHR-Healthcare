...
export default function NurseRequest() {
  ...
  // Filter requests involving a nurse
  const nurseRequests = data.filter(
    (patient) => patient.requestStatus?.type === "Nurse Request"
  );

  // Optional search functionality
  const filteredRequests = nurseRequests.filter((patient) => {
    const term = searchTerm.toLowerCase();
    return (
      patient.name.toLowerCase().includes(term) ||
      patient.requestStatus?.type.toLowerCase().includes(term) ||
      patient.requestStatus?.status.toLowerCase().includes(term)
    );
  });

  return (
    <div className="p-4">
      <h2 className="text-xl font-semibold mb-4">Nurse Requests</h2>
      ...
      {filteredRequests.length === 0 ? (
        <p className="text-gray-500">No nurse requests found.</p>
      ) : (
